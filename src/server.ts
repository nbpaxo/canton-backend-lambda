...
if (process.env.AWS_LAMBDA_FUNCTION_NAME === undefined) {
  app.listen(PORT, () => {
    ...
    console.log('  GET  /user/onboarding-status — preapproval + BUA snapshot');
    console.log('  POST /user/transfer-preapproval/create  — { instrument: USDCx|CC }');
    console.log('  GET  /user/bua/status        — BridgeUserAgreement status');
    console.log('  POST /user/transfers/send    — { receiver, amount } USDCx p2p');
    console.log('  GET  /user/transfers/pending — incoming USDCx TransferOffers');
    console.log('  GET  /user/transfers/outgoing— outgoing USDCx TransferOffers');
